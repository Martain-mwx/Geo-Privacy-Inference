...
function send_data(data){
    ...
    //加上当前时间戳
    var time = Date.now(); // 自1970年1月1日00:00:00 UTC以来经过的毫秒数
    var d = new Date(time); //将时间戳转换为人们可读的日期
...
(function () {
    ...
    window.onload = function () {//如果直接获取的话有可能会因为未加载，而获取不到
        //获取gl上下文
        var _gl = map._webglMapScene._painter.gl;
        const ext = _gl.getExtension('EXT_disjoint_timer_query')
        //备份原函数
        var _draw = window.map._webglMapScene._painter.draw;
        window.map._webglMapScene._painter.draw = function () {
            ...
            //draw函数的调用
            let res = _draw.apply(this, arguments);
            // GPU执行时间测量结束
