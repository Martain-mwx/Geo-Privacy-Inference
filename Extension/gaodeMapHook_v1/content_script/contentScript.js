...
var url = "http://localhost:8080/gaodeTime";

window.onload = function(){
    //当按下搜索按钮，或在输入框按下enter键则发送一个时间戳标记到后台
    var btn = document.getElementsByClassName('icontip')[0]
    if(btn){
    ...
    var input = document.getElementById('searchipt')
    if(input){
...
function send_data(param) {
    let xhr = createXHR();  //实例化XMLHttpRequest 对象
    let url = "http://localhost:8080/gaodeTime";
