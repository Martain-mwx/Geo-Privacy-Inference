...
import copy
#google模型
loaded_model = joblib.load('model_google/clf_google_m_1.m')
count = 8
...
# 处理处理初次分割的时间序列
def handleGoogleRTList(list):
    # 1. 根据 rendertime == -1 则说明这是某一段的结尾要进行分段
    ...
    for i in res:
        ...
        print("最终预测的数据:",data)
        #4. 预测
        print("预测结果：{}".format(loaded_model.predict(data)))  # 输出分类结果标签
